...
int escribir_primer_dato (int semaforos, struct dato_memoria *origen,struct dato_memoria *destino);
...
int crear_ipcs_a_usar(int *mi_memoria,int *mi_semaforo)
{
	...
	//*mi_memoria=shmget(mi_key,sizeof(struct dato_memoria),IPC_CREAT|PERMISOS);
	*mi_memoria=shmget(mi_key,sizeof(struct dato_memoria),IPC_CREAT|IPC_EXCL|PERMISOS);
	...
	if( (*mi_semaforo=semget(mi_key,2,IPC_CREAT|IPC_EXCL|PERMISOS)) == -1)
	{
		if(errno!=EEXIST)
		{
...
int escribir_primer_dato (int semaforos, struct dato_memoria *origen,struct dato_memoria *destino)
{
	struct sembuf mi_semop;

    mi_semop.sem_num=0;    // semaforo Nº0 dentro del set
	mi_semop.sem_op=-1;     // restarle 1 (liberar semaforo)
    mi_semop.sem_flg=0;

	// ya tengo el semaforo tomado, lo inicialize asi solo yo puedo escribir
	*destino=*origen;

    // Ya escribi lo que queria ahora a liberar semaforo
	if(semop(semaforos,&mi_semop,1)) // usa sola operacion
	{
		perror("Error en 1er escritura, al liberar los semaforo \n");
        return(LE_PIFIE_FEO);
	}
	return 0;
}
