...
static void Tarea_Menu (void *punt)
{
	...
	while(1)
	{
		//Liberar la pantalla para que Otros la usen
		//xSemaphoreGive(semBloquearGLCD);

		GLCD_Escribir_Frase((uint8_t*)"Dale, Apreta pue MENU!!!",0,1,Letras_Blancas);
		...
		//Bloquear pantalla para que solo yo la use
		//xSemaphoreTake(semBloquearGLCD,portMAX_DELAY);
