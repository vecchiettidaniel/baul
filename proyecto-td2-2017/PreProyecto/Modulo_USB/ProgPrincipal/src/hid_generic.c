...
extern void USB_Recibir_Dato(USBD_HANDLE_T hUsb, void *data);
/*****************************************************************************
 * Private functions
 ****************************************************************************/
...
//	uint32_t Reads;
	...
	case USB_EVT_OUT:
		/* La funcion USB_funcion_principal() esta declarada como externa
		 * asi puedo armarla donde mejor me parezca y listo
		 * */
		USB_Recibir_Dato(hUsb,data);
		/* Read the new report received. */
//		Reads = USBD_API->hw->ReadEP(hUsb, pHidCtrl->epout_adr, loopback_report);
		/* loopback the report received. */
//		USBD_API->hw->WriteEP(hUsb, pHidCtrl->epin_adr, loopback_report, Reads);
		break;
	}
	return LPC_OK;
}
